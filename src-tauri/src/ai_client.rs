...
pub async fn transcribe_and_clean(
    ...
    match provider {
        ApiProvider::Gemini => {
            ...
            let prompt = format!(
                "You are an elite dictation and editing assistant. Task: Transcribe the audio. Context: The user had this text selected: [SELECTED_TEXT]. Instructions: Clean up the dictation by removing filler words (um, uh, like), correcting grammar, and adding proper punctuation. If the dictation contains a command to edit the selected text (e.g., 'make this formal', 'refactor to async'), perform the edit on the selected text. Return ONLY the final transcribed/edited text. Do not add explanations."
